...
VGADisplay::VGADisplay(/* args */)
{
    if (SDL_Init(SDL_INIT_VIDEO|SDL_INIT_EVENTS) != 0) {
        assert(0);
    }
    main_window = SDL_CreateWindow(VGADisplay_WINDOW_TITLE, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 
                    VGADisplay_WINDOW_WIDTH, VGADisplay_WINDOW_HEIGHT, SDL_WINDOW_SHOWN);
    ...
    main_texture = SDL_CreateTexture(main_renderer, 
                    ...
                    VGADisplay_WINDOW_WIDTH, VGADisplay_WINDOW_HEIGHT);
    reset();
}
...
void VGADisplay::reset()
{
    uint32_t *rgb_p = new uint32_t[VGADisplay_WINDOW_NUMPIX];
    memset(rgb_p, 0, sizeof(uint32_t)*VGADisplay_WINDOW_NUMPIX);
    update(0,0,VGADisplay_WINDOW_NUMPIX,rgb_p);
    delete rgb_p;
    display();
}

void VGADisplay::update(
...
void VGADisplay::update(uint64_t h_addr, uint64_t v_addr, uint32_t rgb)
{
                ...
                uint64_t num, uint32_t *rgb_p)
{
    ...
    int pitch;
    uint32_t *dst;
    assert(h_base_addr>=0&&h_base_addr<VGADisplay_WINDOW_WIDTH);
    assert(v_base_addr>=0&&v_base_addr<VGADisplay_WINDOW_HEIGHT);
